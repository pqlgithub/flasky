# -*- coding: utf-8 -*-
"""
    参考：https://github.com/Jolly23/wx_pay_python
    修复了部分不兼容python3的问题
"""
import hashlib
import random
import string
import time
import urllib.request  # python2 使用urllib2
import urllib.parse
...
class WxPay(object):
    ...
    def __init__(self, wx_app_id, wx_mch_id, wx_mch_key, wx_notify_url, ssl_key=None, ssl_cert=None):
    ...
    def fetch(self, url, data, use_cert=False):
        ...
        if use_cert:
            resp = self.sess.post(url, data=self.to_xml(data), cert=(self.ssl_cert, self.ssl_key))
        else:
        ...
        if 'return_code' in content:
            ...
            if 'result_code' in content and data.return_code == FAIL:
